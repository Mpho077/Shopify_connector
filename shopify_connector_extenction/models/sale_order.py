...
class SaleOrder(models.Model):
    ...
    def _shopify_auto_invoice(self):
        ...
        if ICP.get_param('shopify_connector_extension.auto_create_invoice') != 'True':
            return
        if self.state != 'sale':
            return
        ...
        try:
            ...
            invoice.action_post()
            if ICP.get_param('shopify_connector_extension.auto_register_payment') == 'True':
                journal_id = int(ICP.get_param('shopify_connector_extension.payment_journal_id', '0'))
                if journal_id:
